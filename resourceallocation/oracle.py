...
class Oracle:
    ...
    def allocate_all_processes(self, max_mns_per_process=13, allocation_mode="max_mns", splitting_mode="optimal"):
        if self.context is None:
            raise ValueError("Context not set. Use set_context() to set the context.")

        # Build min_cpu_dict
        min_cpu_dict = {(p, h, 0): 0.0 for p, h in itertools.product(self.context.processes, self.context.hosts)}
        for p, h in itertools.product(self.context.processes, self.context.hosts):
            max_delay_ms = self.context.processes[p].max_delay_ms
            tmp = defaultdict(list)
            for (pp, hh, cpu, mns), val in self.context.links["gamma_tot_precomputed"].items():
                if pp == p and hh == h and val <= max_delay_ms and mns <= max_mns_per_process:
                    tmp[mns].append(cpu)
            for k, vals in tmp.items():
                if vals:
                    min_cpu_dict[(p, h, k)] = min(vals)

        host_caps = {h: float("inf") if host.infinite_parallelism else 1.0 for h, host in self.context.hosts.items()}
        ret = self.solve_mn_allocation(min_cpu_dict, host_caps, allocation_mode=allocation_mode, splitting_mode=splitting_mode)

        split_by_host = {h: [] for h in self.context.hosts}
        for (p, h), m in ret["selected_m_by_pair"].items():
            if m > 0:
                split_by_host[h].append((p, m, float(min_cpu_dict[(p, h, m)])))
        
        return split_by_host
...
if __name__ == "__main__":
    ...
    ret = oracle.allocate_all_processes(
        max_mns_per_process=MAX_MNS, allocation_mode=args.allocation_mode, splitting_mode=args.splitting_mode
    )   
